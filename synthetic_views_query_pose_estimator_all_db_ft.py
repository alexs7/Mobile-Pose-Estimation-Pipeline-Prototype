...
all_db_features = db.get_all_feature_data(row_length)

descs_synth = all_db_features[:, -128:].astype(np.float32)
...
points_query_2D_idx = np.array([good_match.queryIdx for good_match in good_matches])
points_query_2D = kps_query_xy[points_query_2D_idx,:].astype(np.float32)

points_db_3D_idx = np.array([good_match.trainIdx for good_match in good_matches])
points_db_3D = all_db_features[points_db_3D_idx, 2:5].astype(np.float32)

K = np.loadtxt(sys.argv[4])

_, rvec, tvec, _ = cv2.solvePnPRansac(points_db_3D, points_query_2D, K, distCoeffs = np.zeros((5, 1)), iterationsCount = 10000, confidence = 0.99, flags = cv2.SOLVEPNP_EPNP)
...
save_projected_points(points_db_3D, points_query_2D, est_pose_query, K, query_image, verification_image_path)
